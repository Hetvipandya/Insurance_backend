...
exports.loginUser = async (req, res) => {
  try {
    ...
    // ===================== IF USER NOT FOUND → CHECK TEAMLEADER =====================
    if (!user) {
      const tlQuery = isEmail
      ...
      if (!linkedUser) {
        linkedUser = await User.create({
          ...
          emailId: teamLeader.Email,
          mobileNumber: teamLeader.mobileNo,
          address: teamLeader.address || "N/A",
      ...
      if (fcmToken?.trim()) {
        linkedUser.fcmToken = fcmToken.trim();
        await linkedUser.save();
      }

      const safeUser = await User.findById(linkedUser._id).select("-password");
    ...
    if (user.role === "dealer" && !user.isApproved) {
      if (user.isRejected) {
        return res.status(403).json({
          message: `Rejected: ${user.rejectReason || "No reason"}`,
        });
      }
